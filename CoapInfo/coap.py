...
async def findCoapServers(partition_path):
    ...
    try:
        ...
        with open(files_handling.path_dict_to_str(partition_path), newline='') as zmap_file:
            partition_size = sum(1 for _ in csv.reader(zmap_file))

        # 2. zmap partition csv
        with open(files_handling.path_dict_to_str(partition_path), newline='') as zmap_file:

            partition_csv = csv.reader(zmap_file)
            ...
            for row in partition_csv:
                ...
                files_handling.store_data(row, files_handling.path_dict_to_str(coap_test_path))
